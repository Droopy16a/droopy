...
function Box({ title, desc, color = 'indigo', icon = "faCoffee", link = null }) {
  ...
  return (
    <div className="relative group">
      <div className={`absolute inset-0 ${c.bgLight} rounded-lg opacity-0 group-hover:opacity-100 blur-sm transition-opacity`}></div>
      <div className={`border-2 bg-gradient-to-br from-gray-900 ${c.borderLight} rounded-lg p-6 relative z-10 h-full ${c.borderHover} transition-all duration-300`}>
        <div className="flex flex-col h-full">
          <div className="flex items-center mb-4">
            <div className={`w-10 h-10 rounded ${c.iconBg} flex items-center justify-center mr-3 transition-colors`}>
                <FontAwesomeIcon icon={icon} className={c.icon}/>
            </div>
            <div className={`font-medium text-lg ${c.text} transition-colors`}>{title}</div>
          </div>
          <p className="text-start text-gray-400 mb-4">{desc}</p>
          { link ? <a href={link} className={`mt-auto ${c.link} inline-flex items-center`}>
            Learn more
